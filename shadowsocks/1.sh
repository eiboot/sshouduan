...
python2.7.16(){
	...
	echo -e "${OK} ${GreenBG} 尝试  安装 python2.7.16 ${Font}"
	    cd /usr/bin
	    sleep 2
	    echo -e "${OK} ${GreenBG} 尝试 下载python2.7.16 ${Font}"
		wget https://www.python.org/ftp/python/2.7.16/Python-2.7.16.tgz
		sleep 2
		tar -zxvf Python-2.7.16.tgz
		cd /usr/bin/Python-2.7.16
		sleep 2
		echo -e "${OK} ${GreenBG} 尝试  编译 python2.7.16 ${Font}"
          ./configure --prefix=/usr/local/python-2.7.16
          make
          make install
          rm /usr/bin/python
          ln -s /usr/local/python-2.7.16/bin/python /usr/bin/python
          echo -e "${OK} ${GreenBG} 完成安装 python2.7.16 ${Font}"
          rm -rf /usr/Python-2.7.16.tgz
          echo -e "${OK} ${GreenBG} 删除python2.7.16压缩包 ${Font}"
	fi
}
		...
		if [[ $? -ne 0 ]]; then
			if [[ ${ID} == "centos" ]];then
				...
				yum -y install python-pip 
			     ...
			     echo -e "${OK} ${GreenBG} 尝试 apt 安装 python2.7 ${Font}"
				sleep 2
				apt-get install -y python2.7
				echo -e "${OK} ${GreenBG} 将python2.7连接上python ${Font}"
				sleep 2
				sudo ln -s /usr/bin/python2 /usr/bin/python
				echo -e "${OK} ${GreenBG} 尝试 apt 安装 git wget ${Font}"
				sleep 2
...
SSR_dependency_installation(){
	if [[ ${ID} == "centos" ]]; then
		...
		pip install requests
		sleep 2
		rm -rf CyMySQL && rm -rf cymysql && git clone https://github.com/nakagami/CyMySQL.git && mv CyMySQL/cymysql ./ && rm -rf CyMySQL
	fi
}
...
common_set(){
	...
	stty erase '^H' && read -p "SPEEDTEST_CIRCLE(num_only,default:6):" SPEEDTEST
	[[ -z ${SPEEDTEST} ]] && SPEEDTEST="6"
	stty erase '^H' && read -p "CLOUDSAFE_ON(0 or 1,default:1,advise:0!):" CLOUDSAFE
	...
	[[ -z ${WEBAPI_URL} ]] && WEBAPI_URL="https://zhaoj,in"
	stty erase '^H' && read -p "WEBAPI_TOKEN(example: zhaoj.in):" WEBAPI_TOKEN
	[[ -z ${WEBAPI_TOKEN} ]] && WEBAPI_TOKEN="glzjin"
...
#basic install	
	...
	cd ${shadowsocks_install_folder} && git clone -b manyuser https://github.com/NimaQu/shadowsocks.git /root/shadowsocks
